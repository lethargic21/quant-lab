...
- **누적**: `data/raw/toss/{code}/_cumulative.parquet`
  ...
  - `is_deleted` = 직전 크롤엔 있었는데 이번 관측창에서 사라진 글 → **삭제 신호**
    ...
    - ⚠️ **라이브 누적(`_cumulative.parquet`)의 is_deleted는 계속 비활성**(항상 False). 전역
      id경계 판정은 대량 오탐(실측: 000660 68글에 736건). 라이브 크롤은 순수 관측만 한다.
  - `likes/comments` = 크롤마다 갱신(시계열은 스냅샷들에 보존).
  - 스냅샷은 이제 `feed_rank`(피드 순서 = 삭제탐지 브래킷의 기반) + `content_hash`(편집 탐지)를
    **명시 컬럼**으로 저장한다(additive — 구 스냅샷은 파생 단계가 행순서·title로 보완).
...
## 삭제탐지 (오프라인 파생 — `toss/deletions.py`)

삭제는 **불변 스냅샷에서 오프라인으로** 판정한다(라이브 크롤 무관 → 크롤 부하 증가 0,
재실행 idempotent). `python -m dart_event_study.toss.deletions`가 스냅샷에서 4테이블을 파생:

- `posts` — post_id PK, stock, author_hash, created_at_reported(상대라벨), first_seen,
  last_seen, content_hash, n_content_hashes/edited, **status**(alive|deleted|censored), deleted_at_inferred
- `observations` — poll_id, post_id, **feed_rank**, observed_at, content_hash
- `polls` — poll_id, stock, started_at/ended_at, depth_reached, page_count, newest/oldest_post_seen, status
- `survival` — post_id, first_seen, end_time, **event(1 삭제 / 0 우측절단)**, duration_min (KM 등 바로 태울 형태)

**판정 규칙 — absence ≠ deletion (핵심)**: 전역 [min,max] id경계가 아니라 **피드-랭크 지역
인접성**. X가 마지막 관측된 poll에서 위 이웃 A(더 최신)·아래 이웃 B(더 오래)를 가졌을 때,
이후 어느 poll에서 **A·B가 둘 다 관측 + 피드상 인접(rank 차 1)** 이고 X가 없으면 → 자리가
닫힘 = **삭제 확정**. A/B가 크롤 깊이 밖이면 → **censored**(절대 삭제 아님). 고정글(pinned)이
id범위를 수백만 벌려도 X의 실제 이웃만 보므로 오탐하지 않는다. 다중삭제(이웃도 삭제)는
보수적 censored(과소계상 허용, 과대계상 금지). 실측: 20종목 28,047글 → 삭제확정 33(0.1%),
나머지는 대부분 censored(4슬롯/일 재관측 깊이 밖으로 밀림). **삭제율은 지표가 아니라 raw
관측 산물** — 좌측절단·크롤깊이 편향 미보정.

**재폴링 현실**: 라이브 크롤이 매 슬롯 최신 피드를 재관측하므로(신규글 촘촘) 게시 직후
삭제는 잡히지만, 고활성 종목은 수시간 내 재관측 깊이 밖으로 밀려 censored가 지배적이다.
삭제(주로 게시 직후 발생)를 더 잡으려면 최근글 재폴링 밀도를 올려야 하나, 이는 크롤 부하·
스케줄 변경이라 별도 승인 후. 현재 탐지는 부하 0(기존 관측 재사용).
