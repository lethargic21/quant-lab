...
- **상호작용 `attn:is_trust`는 4개 스펙 전부 p≥0.67, 부호도 불안정** → 타입별 어텐션 효과
  차이 증거 없음.
- 어텐션 주효과는 **겹치는 스펙 하나에서만** 유의(p=.015), 겹치지 않는 [+2,+20]에선 소멸
  (p=.13/.46) → 사전 규칙에 따라 **동시성으로 판정, 예측력 아님**. R² 0.002~0.066.
- 부수 발견: **log 시총 계수가 음수로 유의**(p=.002/.024) — 소형주일수록 드리프트가 큼.
  기사 수가 대형주에 몰리므로 규모를 통제해야 어텐션 계수가 드러난다 → §11 삼분위의
  단조 패턴 상당 부분이 **규모 교락**이었음을 시사.
