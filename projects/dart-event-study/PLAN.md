...
## 9. Phase 4 디버그 결과 (5종목 — 예비 관찰, full 실행 전)

**이벤트 스터디** (시장모형, 추정 120일/갭 10일):
- 자사주 [-1,+1] +2.44% (t=2.5, N=20 유보) / 실적+ [-1,+1] +1.13% (t=2.6, N=123)
- 유증− [0,+20] −11.9% (t=−6.1, N=4 유보) — 방향 룰과 부호 전부 일치
- 공시창 반응은 뚜렷, [0,+20] 드리프트는 약함

**백테스트** (익영업일 종가 진입, 비용 반영, H=5/20 전수 리포트):
- 전 조합 순수익 음수 (all H=5 long-short 연 −22.9%, KOSPI +3.1%)
- 해석: 보수적 타이밍(익영업일 진입)이 공시창 점프를 놓치고, 드리프트는 비용을
  못 이김. 디버그 5종목은 2021~24 급락 종목(카카오·셀트리온) 편중 — full에서 재검증.
- 이벤트 스터디(정보력)와 백테스트(체결가능 수익)의 괴리 자체가 핵심 발견:
  "공시 정보는 가격에 빠르게 반영되며, 다음날 진입으로는 초과수익을 얻기 어렵다"

**full 실행 비용 추정**: API 호출 ~1.5만—OpenDART 일한도 2만 내이나 근접.
잠정실적 원문 ~8,400건 다운로드 포함 실행 시간 수십 분 예상.

---

## 11. 감성 레이어 설계 (2단계 — 2026-07-12 착수)

**가설 (사전 등록)**: 자사주 공시(직접 178 + 신탁 106 = 284건)의 **뉴스 반응 크기**에 따라
이후 드리프트가 다른가.
- H-되돌림: 반응 상위 그룹의 [0,+20] CAR < 하위 그룹 (과잉반응 후 되돌림)
- H-모멘텀: 반응 상위 그룹의 CAR > 하위 그룹 (관심 지속)
사전 고정: 대상 = 자사주 이벤트만(유일하게 강건한 신호), 윈도우 = [0,+20] 하나
(기존 유의 윈도우 — 다중검정 최소화), 그룹 = 반응 지표 삼분위, 검정 = 그룹 차 t +
월클러스터 SE. 결과가 어느 방향이든/무의미하든 그대로 리포트.

**데이터 소스 (실측 완료)**: 네이버 뉴스 검색 날짜필터(`ds/de`) — 2020년 과거 기사
반환 확인, 봇차단 없음. 쿼리 = "회사명 + 자사주", 창 = 접수일 [-1, +1].
스로틀 0.5~1초 + 디스크 캐시 (284 이벤트 × 페이지네이션 ≈ 1~2천 요청, 30~60분).
리스크: 신형 마크업(해시 클래스) — 구조 기반 파싱, 변경 취약성 문서화.

**반응 지표 옵션**:
- A. 기사 수 (attention) — 모델 불필요, 단순·해석 가능. 교락(대형주=기사 많음)은
  프록시 시총과의 상관을 병기해 정직 공개
- B. A + 제목 감성 룰 (극성 사전 기반 — 호재성/악재성 단어) ← **채택 (2026-07-12)**
- C. 한국어 사전학습 모델 감성 (KcELECTRA 계열) — 무겁고 재현 부담, 룰이 부족할 때만

**결과 (2026-07-12, 273건 매칭)**:
- attention 삼분위 CAR[0,+20]: 하위 +3.11% / 중위 +4.91% / 상위 +5.74% — 단조 증가
  (모멘텀 방향)이나 상-하위 차이 +2.63%p는 Welch t=1.54, p=0.13으로 **비유의**.
  되돌림 가설은 기각 방향, 모멘텀 가설은 힌트 수준.
- 제목 감성 삼분위: 차이 p=0.54 — 무정보. 자사주 뉴스는 평균적으로 호의적(+0.30)이라
  톤 변별력이 없음.
- 교락: 기사수×발행총수 Spearman ρ=0.19 (약함). 세 그룹 모두 드리프트 자체는 유의.
- 실측 이슈: 네이버 소프트 403 (55이벤트째) → 스로틀 2.5s+지터, 403 백오프로 해결.
  쿼리("회사명 자사주") 정밀도 한계 — 최소 기사 수 5건은 노이즈 플로어 존재 시사.

---

## 10. Full 실행 결과 (시총상위 200, 2019–2024 — 2026-07-10, 총 29분)

**표본**: 공시 139,088건 → 이벤트 5,078 (자사주 174 / 실적 4,803 / 유증 101)
→ 시그널 4,370. 실적 방향 판정 불가 13.7% (디버그 4.3%보다 높음 — 원문 미제공
+ 표 형식 변형; 한계로 기록). 유증 배정방식: 제3자 56 / 주주배정후실권주 28 /
일반공모 12 / 주주배정 5.

**이벤트 스터디** (전 윈도우, N≥30 그룹):

| 그룹 | [-1,+1] | [0,+5] | [0,+20] |
|---|---|---|---|
| 자사주+ (N=166) | +3.06% (t=8.1) | +3.75% (t=7.0) | **+4.68% (t=5.3)** |
| 실적+ (N=2325) | +1.29% (t=11.6) | +0.63% (t=4.4) | +0.24% (n.s.) |
| 실적− (N=1664) | −0.60% (t=−4.8) | −0.89% (t=−5.6) | −0.43% (n.s.) |
| 유증− (N=36) | −1.53% (n.s.) | −3.33% (t=−2.6) | −2.03% (n.s.) |

핵심: **자사주만 공시 후 20거래일 드리프트가 유의**. 실적 반응은 [0,+5] 안에 소화.

**백테스트** (익영업일 종가 진입, 왕복비용 0.4%):
- 자사주 H=5: 순 +5.1%/년, Sharpe 0.21, hit 55% (KOSPI +3.1%, Sharpe 0.16) —
  유일하게 비용 생존. 단 N=171 트레이드, MDD −36%로 우위는 미미.
- 실적: 회전율 44~56x/년 → 비용이 전부 잠식 (all gross H=20 롱온리 +14.4%/년이
  net +0.7%로 붕괴). 고빈도 이벤트는 이 비용 가정에서 거래 불가 결론.
- 유증 숏: N=46, H=20 net −33% — 유보 (숏 현실성 문제도 겹침).

**정직한 결론(초안)**: 공시 정보력은 실재(이벤트 스터디)하나, 익영업일 진입이라는
보수적 체결 가정 + 현실적 비용 아래 체계적 초과수익은 자사주 이벤트에서만
미미하게 남는다. 수익률 자랑이 아니라 이 괴리의 정량화가 이 프로젝트의 기여.

---

## 12. CAAR 검정 배터리 (Phase B — 2026-07-18)

**동기**: 기존 유의성은 naive 단면 t + 클러스터-로버스트 SE + BH-FDR까지였다. 이벤트
스터디 표준 검정(BMP·비모수·placebo)을 채워 헤드라인 결과(자사주 드리프트)를 여러 각도로
재검증한다. `event_study.py`는 유닛테스트가 걸린 안정 모듈이라 **건드리지 않고**, 별도
`analysis/caar_tests.py`로 추가한다 (기존 분석 CLI 분리 패턴과 일관).

**설계 결정**:
1. **BMP (Boehmer-Musumeci-Poulsen 1991)**: 각 CAR를 자기 추정기간 예측오차 SD로 표준화
   (SCAR)한 뒤 SCAR의 **단면** t. 예측오차 분산은 Campbell-Lo-MacKinlay 식
   `s²·[L + L²/T + (Σ_win(R_m−R̄_m))²/SSE_m]` (α·β 추정오차항 포함). 단면 표준화가
   event-induced variance를 흡수한다.
2. **Corrado 순위검정 (1989 / Corrado-Zivney 1992 누적형)** + **일반화 부호검정 (Cowan 1992,
   귀무 양수확률 = 추정기간 실제 양수비율)**: 비모수 — 정규성·분산 가정에 강건. 얇거나
   꼬리 두꺼운 표본(유증 N=49)에서 파라미터 검정과 갈릴 수 있고, 갈리면 그대로 리포트.
3. **CAAR event-level 부트스트랩 CI**: 이벤트를 복원추출해 평균 CAR의 표본 불확실성.
4. **Placebo / permutation (제일 중요)**: 각 이벤트를 **같은 종목의 랜덤 거래일**로 치환
   (실제 이벤트 ±30거래일 buffer 제외 — 실제 효과 누출 방지)해 동일 파이프라인을 1,000회
   돌려 CAAR 귀무분포를 만든다. 경험적 양측 p = (1+#{|null|≥|real|})/(n_perm+1). 종목 구성·
   표본 수를 실제와 동일하게 두고 **타이밍만** 무작위화 → "이 효과가 아무 날에나 나오나?".
   성능: 종목별 누적합(prefix-sum) 닫힌형 OLS로 CAR을 O(1) 계산·벡터화 (순진한 이중 루프
   대비 수십 배). 결과가 `car_fast`(→`market_model_ar`)와 일치함을 유닛테스트로 가드.
5. **다중검정**: BMP·placebo p에 표 전체(그룹×윈도우) BH-FDR q값 병기.

**결과 (n_perm=1,000, full)**: 자사주 직접·신탁 드리프트는 placebo 포함 전 검정 통과
(placebo 귀무평균 ≈ 0, q<.01). **실적+ [0,+20]은 BMP·부호가 유의로 보이나 placebo가 위양성을
기각** — placebo 귀무평균 +0.11% ≈ 실제 +0.10%(placebo p=.56, Corrado p=.97)로 이벤트 효과가
아니라 기저 드리프트. **유증−은 저검정력 BMP(N=49, p=.14)가 놓친 효과를 Corrado·부호·placebo가
포착**(placebo q<.05). 방향 결론은 유지, 근거만 두꺼워짐. 상세 표는 README §결과 1b.

**가정·한계**: (a) placebo는 종목별 독립 무작위 → 실제 이벤트의 달력 클러스터링(같은 날 여러
종목)은 재현 안 함 — 그 축은 event_study의 클러스터-로버스트 SE가 담당(역할 분리). (b) Corrado는
매트릭스 정렬을 위해 추정표본이 정확히 est_len인 이벤트만 사용(n_corrado로 리포트). (c) 잔차분산
0인 퇴화 이벤트의 SCAR는 nan → BMP에서 자동 제외.

---

## 13. 이벤트 × 어텐션 상호작용 회귀 (Phase A — 2026-07-20)

**동기**: §11의 어텐션 분석은 삼분위 portfolio sort까지였다. 지시서 A-2의 단면 회귀
(`CAR ~ event_type + attn + event_type:attn + controls`)로 승격한다.
구현: `analysis/interaction.py`, `statsmodels` 없이 numpy로 CR1 클러스터-로버스트 OLS
(`event_study.clustered_t`와 같은 계열 — 의존성 추가 대신 기존 방식과 일관).

**설계 결정 / 사전 등록**:
1. **event_type 축의 실제 범위**: 뉴스 어텐션은 자사주에만 수집돼 있어(실적·유증 미수집)
   타입 축은 **직접취득(169) vs 신탁계약(104)** 두 그룹뿐. 전 이벤트 타입 비교가 아님을 명시.
2. **어텐션의 시점**: 수집창이 접수일 [-1,+1]이라 **사전 관심이 아니라 사후·동시 반응**.
   변수명을 `attn_post`로 못박음.
3. **종속변수 2개를 사전 등록**: `CAR[0,+20]`(기존 윈도우, 어텐션 창과 **겹침**) +
   `CAR[+2,+20]`(**겹치지 않음**). 판정 규칙도 사전 고정 — *겹칠 때만 유의하고 안 겹칠 때
   무의미하면 예측력이 아니라 동시성*. 윈도우를 바꿔가며 유의성을 찾지 않기 위한 장치.
4. **통제**: log 시총(= 발행총수 × day0 종가), 추정창 잔차 변동성. 시총 결측 41건 때문에
   통제 O(N=234)/통제 X(N=273) 두 스펙을 **둘 다** 리포트.
5. **abnormal attention의 한계**: 원안은 "종목별 rolling baseline 대비 z-score"지만 수집
   데이터가 이벤트당 기사 수 하나뿐이라 **일별 baseline 시계열이 없다** → log1p + 표본
   z-score + 회귀 내 규모 통제로 대체. 진짜 rolling baseline이 아님을 문서화.

**결과 — null (상세 표는 README §감성 레이어)**:
- **1차 (자사주 직접 vs 신탁, N=273)**: `attn:is_trust` 4개 스펙 전부 p≥0.67 → 타입별 차이 없음.
  어텐션 주효과는 겹치는 스펙에서만 유의(p=.015), 겹치지 않는 [+2,+20]에선 소멸 → 동시성 판정.

**확장 (2026-07-21): 유증 추가 → 3타입 재적합**
- 뉴스 어텐션을 유상증자 139건까지 수집(검색어 "유상증자", 기사 중앙값 11·0건 없음). event_type을
  더미로 일반화(기준=자사주 직접, `run_spec`이 타입별 더미·상호작용 자동 생성). **실적 5,300건은
  스로틀상 ~10h·밴 위험으로 제외**(collect.py GROUPS에 주석). 규모 통제는 유증에 total_shares가
  없어 pre_shares(증자 전 발행총수)로 폴백.
- **상호작용 여전히 null, 더 견고**: `attn:trust` p≥.70, `attn:rights` p≥.22 (4개 스펙 전부).
  방향이 반대인 유증(희석 악재)을 넣어도 타입별 어텐션 효과 차이 증거 없음.
- **유증 더미 −6.5%p (p<.001)** — 모델이 알려진 희석 악재 드리프트를 정확히 포착(검증). 유증
  방향 혼합(제3자65/악재50/호재24)인데 평균이 강한 음 → 악재 배정이 표본 지배.
- 어텐션 주효과는 유증 추가로 p=.015→.061(한계적)로 약화. 규모 교락은 약해지고(log시총 p=.13)
  추정창 변동성이 유의한 통제로 등장(p=.013).

**결론**: 호재(자사주)·악재(유증)를 아우른 표본에서도 "어텐션이 드리프트를 타입별로 다르게
증폭/소멸시키는가"의 답은 **아니오(null)**. 유의하게 만들려 스펙을 바꾸지 않았다.
